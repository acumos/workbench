...
module.exports = function(app) {
	...
	app.post('/api/model/count', function(req, res) {
		...
		getModelCount(userName, serviceUrl, getLatestAuthToken(req, authToken), reqBody).then(function(result) {
			res.send(result);
		});
	});
	...
	app.post('/api/prmodel/count', function(req, res) {
		...
		let authToken = req.headers['auth'];
		let reqBody = req.body.reqBody;
		getPRModelCount(userName, serviceUrl, getLatestAuthToken(req, authToken), reqBody).then(function(result) {
			res.send(result);
		});
	});
	...
	var getModelCount = function(userName, url, authToken, reqBody) {
		return new Promise(function(resolve, reject) {
			var options = {
				method : "POST",
			...
			request.post(options, function(error, response) {
				...
				if (!error && responseData.status_code == 0) {
					let modelCount = responseData.response_body.totalElements;
	...
	var getPRModelCount = function(userName, url, authToken, reqBody) {
		return new Promise(function(resolve, reject) {
			var options = {
				method : "POST",
			...
			request.post(options, function(error, response) {
				var prResponseData = JSON.parse(response.body);
				if (!error && prResponseData.status_code == 0) {
					let prModelCount = prResponseData.response_body.totalElements;
