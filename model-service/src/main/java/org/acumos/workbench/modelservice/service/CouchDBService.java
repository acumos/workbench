...
import org.acumos.workbench.modelservice.exceptionhandling.ProjectModelAssociationNotFoundException;
...
public class CouchDBService {
	...
	/**
	 * Insert Project Model Association in Couch DB
	 * @param modelData
	 * 			the DataSetModel
	 * @param model
	...
	/**
	 * Insert ProejctModel Association in CouchDB 
	 * @param modelData
	 * 			the DataSetModel
	 * @param modelName
	...
	/**
	 * Retrieve the list of models associated to a project, owner as input user
	...
	public List<DataSetModel> getModelsAssociatedToProject(String authenticatedUserId, String projectId) {
		String jsonQuery = "{\"selector\":{\"$and\":[" + "{\"ProjectId\":{\"$eq\":\"" + projectId + "\"}},"
	...
	public DataSetModel getAssociatedModel(String associatedId) {
		...
		try {
			...
			logger.error("Error while fetching association from CouchDB");
			throw new ProjectModelAssociationNotFoundException();
		} finally {
			try {
				// closing the resources
				dbClient.close();
	...
	private void associationExistsInCouch(String projectId, String solutionId, String revisionId)
		...
		String jsonQuery = "{\"selector\":{\"$and\":[" + "{\"ProjectId\":{\"$eq\":\"" + projectId + "\"}},"
				+ "{\"SolutionId\":{\"$eq\":\"" + solutionId + "\"}}," + "{\"RevisionId\":{\"$eq\":\"" + revisionId
