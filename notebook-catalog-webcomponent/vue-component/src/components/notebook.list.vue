<template>
  <div class="flex flex-col m-3">
    <div class="flex w-full justify-end">
      <button class="btn btn-primary" @click="editNotebook()">
        Create Notebook
      </button>
      <a :href="notebookWikiURL" target="_blank" class="btn btn-secondary ml-2"
    ...
    <div class="flex flex-wrap">
      <NotebookCard
        ...
        :key="index"
        :notebook="notebook"
        @on-open-project="$emit('on-open-notebook', $event)"
      />
    </div>
    <div>
      <pagination-ui
...
export default {
  ...
  computed: {
    ...
    filters() {
      return {
        ...countBy(this.notebooks, "type"),
        all: this.notebooks.length
      };
    },
    sortOptions() {
      if (this.sortBy === "") {
