...
export class NotebookLitElement extends DataMixin(ValidationMixin(BaseElementMixin(LitElement)))  {
		...
		renderViewNotebook(notebook){
			this.$data.set('notebook.notebookName', notebook.noteBookId.name, true);
		  this.$data.set('notebook.notebookCreateDate', notebook.noteBookId.versionId.timeStamp, true);
		  this.$data.set('notebook.notebookDesc', notebook.description, true);
	    this.$data.set('notebook.notebookModifyDate', notebook.noteBookId.versionId.timeStamp, true);
      this.$data.set('notebook.notebookVersion', notebook.noteBookId.versionId.label, true);
			...
			fetch(url, {
				...
				.then((n) => {
					if(n.status === 'Success'){
						...
						this.notebookName = this.data.notebook.notebookName;
						this.$validations.resetValidation('notebook');
					...
					<div class="row ">
						<div class="col-md-12 py-3">
									...
									<div class="row" style="margin:5px 0; margin-top: 0px;">
										...
										${this.data.notebook.notebookStatus === 'ACTIVE'
											? html`
												${this.isEdit 
													? html`
														${this.$validations.$valid('notebook') && this.$validations.$dirty
                              ? html`
                            ...
                            `}
                              ...
                              @keyup=${(e) => this.$data.set('notebook.notebookDesc', e)}>
                            </textarea>
													</td>
													`
													: html`
