...
import org.acumos.workbench.pipelineservice.util.PipeLineLoggingAdapter;
...
public class LoggingHandlerInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		PipeLineLoggingAdapter psLoggingAdapter = new PipeLineLoggingAdapter(
				LoggerFactory.getLogger(MethodHandles.lookup().lookupClass()));
		psLoggingAdapter.entering(request);
		return true;
	}
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
		...
		final PipeLineLoggingAdapter psLoggingAdapter = new PipeLineLoggingAdapter(
				LoggerFactory.getLogger((MethodHandles.lookup().lookupClass())));
		psLoggingAdapter.exiting();
	}
