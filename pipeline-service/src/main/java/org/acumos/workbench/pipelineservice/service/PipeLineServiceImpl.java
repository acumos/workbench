...
public class PipeLineServiceImpl implements PipeLineService{
	...
	public Pipeline createPipeLine(String authenticatedUserId, String projectId, Pipeline pipeLine) throws TargetServiceInvocationException {
		...
		logger.debug("CDS createPipeLine() method End");
			...
			logger.warn("CDS - Create Pipeline");
			throw new TargetServiceInvocationException(props.getCdsCreatePipelineExcp());
		}
		try {
			if(null != projectId){
	...
	public void pipeLineExists(String authenticatedUserId, String projectId, Pipeline pipeLine)
		...
		if (null != response && response.getContent().size() > 0) {
			logger.warn("Pipeline name and version already exists");
	...
	public boolean isProjectArchived(String projectId) throws ArchivedException {
		...
		logger.debug("CDS getProject() method Begin");
		MLPProject mlpProject = cdsClient.getProject(projectId);
		logger.debug("CDS getProject() method End");
	...
	public boolean isOwnerOfProject(String authenticatedUserId, String projectId) throws NotOwnerException {
		...
		String userId = mlpUser.getUserId();
		logger.debug("CDS getProject() method Begin");
		MLPProject response = cdsClient.getProject(projectId);
		logger.debug("CDS getProject() method End");
		if (null == response) {
			logger.warn("Requested Project Not found");
			throw new ProjectNotFoundException();
	...
	public List<Pipeline> getPipelines(String authenticatedUserId, String projectId) {
		...
		if(null == projectId){
			try {
				...
				logger.warn("CDS - Search Pipelines");
				throw new TargetServiceInvocationException(props.getCdsSearchPipelinesExcp());
			}
		}else {
			try {
				...
				logger.warn("CDS - Get Project Pipelines");
				throw new TargetServiceInvocationException(props.getCdsGetProjectPipelinesExcp());
			}
		}
	...
	public boolean isOwnerOfPipeline(String authenticatedUserId, String pipelineId)throws NotOwnerException {
		...
		try {
			...
			if(null == response) {
				logger.warn("Requested Pipeline Not found");
			...
			logger.warn("CDS - Get Pipeline");
			throw new TargetServiceInvocationException(props.getCdsGetPipelineExcp());
		}
	...
	public Pipeline updatePipeline(String authenticatedUserId, String projectId, String pipelineId, Pipeline pipeLine)
		...
		try {
			...
			logger.warn("CDS - Update Pipeline");
			throw new TargetServiceInvocationException(props.getCdsUpdatePipelineExcp());
		}
	...
	public ServiceState deletePipeline(String pipelineId) {
		...
		try {
			...
			logger.warn("CDS - Get Pipeline Projects");
			throw new TargetServiceInvocationException("CDS - Get Pipeline Projects");
		}
		try {
			...
			logger.warn("CDS - Drop Project Pipeline");
			throw new TargetServiceInvocationException(props.getCdsDropProjectPipelineExcp());
		}
		try {
			...
			logger.warn("CDS - Delete Pipeline");
			throw new TargetServiceInvocationException(props.getCdsDeletePipelineExcp());
		}
	...
	public void isPipelineArchived(String pipelineId) throws ArchivedException{
		...
		try {
			...
			if (null != mlpPipeline && !mlpPipeline.isActive()) {
				logger.warn("Pipeline is archived, launching Pipeline is not allowed");
	...
	public void isPipelineAssociatedUnderProject(String projectId, String pipelineId) {
		...
		if (!associated) {
			logger.warn("Specified Project and Pipeline are not associated");
