...
import org.acumos.workbench.pipelineservice.util.PipelineServiceLogConstants.MDCs;
import org.acumos.workbench.pipelineservice.util.PipelineServiceLogConstants.ResponseSeverity;
 ...
 * facade.
 */
public class PipeLineLoggingAdapter {
	 ...
	 *            non-null logger.
	 */
	public PipeLineLoggingAdapter(final Logger logger) {
	 ...
	 * @return this.
	 */
	public PipeLineLoggingAdapter entering(final RequestAdapter<?> request, String format, Object... arguments) {
	 ...
	 * @return this.
	 */
	public PipeLineLoggingAdapter entering(final HttpServletRequest request) {
	 ...
	 * @param format
	 * @param arguments
	 * @return PipeLineLoggingAdapter
	 */
	public PipeLineLoggingAdapter entering(final HttpServletRequest request, String format, Object... arguments) {
	 ...
	 * @return this.
	 */
	public PipeLineLoggingAdapter exiting() {
	 ...
	 * @return this.
	 */
	public PipeLineLoggingAdapter setResponseDescriptor(final ResponseDescriptor d) {
	 ...
	 * @return this.
	 */
	public PipeLineLoggingAdapter setEnteringMDCs(final RequestAdapter<?> request) {

		// Extract MDC values from standard HTTP headers.
		final String requestID = defaultToUUID(request.getHeader(PipelineServiceLogConstants.Headers.REQUEST_ID));
		final String partnerName = defaultToEmpty(request.getHeader(PipelineServiceLogConstants.Headers.PARTNER_NAME));
		final String targetEntity = defaultToEmpty(request.getHeader(PipelineServiceLogConstants.MDCs.TARGET_ENTITY));
		final String targetServiceName = defaultToEmpty(request.getHeader(PipelineServiceLogConstants.MDCs.TARGET_SERVICE_NAME));

		// Get the UserName
		final String userName = defaultToEmpty(request.getUser());
		MDC.put(PipelineServiceLogConstants.MDCs.USER, userName);

		PipeLineLoggingAdapter.setDefaultMDCs();

		// Set standard MDCs.
		MDC.put(PipelineServiceLogConstants.MDCs.REQUEST_ID, requestID);
		if (!partnerName.isEmpty())
			MDC.put(PipelineServiceLogConstants.MDCs.PARTNER_NAME, partnerName);
		MDC.put(PipelineServiceLogConstants.MDCs.CLIENT_IP_ADDRESS, defaultToEmpty(request.getClientAddress()));
		MDC.put(PipelineServiceLogConstants.MDCs.SERVER_FQDN, defaultToEmpty(request.getServerAddress()));

		MDC.put(PipelineServiceLogConstants.MDCs.TARGET_ENTITY, targetEntity);
		MDC.put(PipelineServiceLogConstants.MDCs.TARGET_SERVICE_NAME, targetServiceName);
		return this;
	}
	...
	public static class ResponseDescriptor {
		...
		protected PipelineServiceLogConstants.ResponseStatus mStatus;
		 ...
		 * @return this.
		 */
		public ResponseDescriptor setResponseStatus(final PipelineServiceLogConstants.ResponseStatus status) {
			this.mStatus = status;
			return this;
		}
		...
		public void setMDCs() {
			...
			if (!response.isEmpty())
				MDC.put(PipelineServiceLogConstants.MDCs.RESPONSE_CODE, response);
			if (!description.isEmpty())
				MDC.put(PipelineServiceLogConstants.MDCs.RESPONSE_DESCRIPTION, description);
			if (!severity.isEmpty())
				MDC.put(PipelineServiceLogConstants.MDCs.RESPONSE_SEVERITY, severity);
			if (!status.isEmpty())
				MDC.put(PipelineServiceLogConstants.MDCs.RESPONSE_STATUS_CODE, status);
		}
	}
	...
	public static class HttpServletRequestAdapter implements RequestAdapter<HttpServletRequestAdapter> {
		...
		public Object getUser() {
			return this.mRequest.getAttribute(PipelineServiceLogConstants.MDCs.USER);
		}
	}
}
