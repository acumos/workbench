...
public class PredictorProjectAssociationServiceImpl implements PredictorProjectAssociationService{
		...
		try {
			...
			if (null != predictorProjAssociationList && !predictorProjAssociationList.isEmpty()) {
				for (PredictorProjectAssociation association : predictorProjAssociationList) {
					String predicotrId = association.getPredcitorId();
					String predictorVersion = association.getPredictorVersion();
					// TODO : Need to delete the commented code
					// then call the predictor manager service to check the deployment status is active or not 
					// for the predictorId and PredictorVersion, and get the deployment status 
					// if(!deploymentstatus.equals("ACTIVE")){ then update the PredictorProjectAssociation table as
					//association.setPredictorDeploymentStatus("Error/Failed");
					//association.setAssociationStatus("INVALID");
					//then update the same in couch DB as
					//couchDbService.update(association); }
			...
			logger.error("Exception occured in getPredictors()");
		}
	...
	public Predictor associatePredictorToProject(String authenticatedUserId, String predictorId, String projectId, PredictorProjectAssociation predProjAssociation) {
		...
		Predictor predictor = null;
		try {
			result = couchDbService.associatePredictorToProject(authenticatedUserId,predictorId,projectId,predProjAssociation);
	...
	@Override
	public Predictor editPredictorAssociationToProject(String authenticatedUserId,
		...
		PredictorProjectAssociation newAssociation = null;
		Predictor predictor = null;
		String newVersion = predictorProjAssociation.getPredictorVersion();
		MLPUser mlpUser = getUserDetails(authenticatedUserId);
		try {
			PredictorProjectAssociation oldAssociation = couchDbService.getAssociatedPredictor(associationId);
			String oldVersion = oldAssociation.getPredictorVersion();
			if(null != newVersion && !newVersion.equals(oldVersion)){
				newAssociation = new PredictorProjectAssociation();
				newAssociation.set_rev(oldAssociation.get_rev());
				newAssociation.setAssociationID(associationId);
				newAssociation.setAssociationStatus(oldAssociation.getAssociationStatus());
				newAssociation.setCreatedTimestamp(oldAssociation.getCreatedTimestamp());
				newAssociation.setEnvironmentPath(oldAssociation.getEnvironmentPath());
				newAssociation.setMetadata1(oldAssociation.getMetadata1());
				newAssociation.setMetadata2(oldAssociation.getMetadata2());
				newAssociation.setPredcitorId(predictorProjAssociation.getPredcitorId());
				newAssociation.setPredictorDeploymentStatus(oldAssociation.getPredictorDeploymentStatus());
				newAssociation.setPredictorDescription(oldAssociation.getPredictorDescription());
				newAssociation.setPredictorName(oldAssociation.getPredictorName());
				newAssociation.setPredictorVersion(newVersion);
				newAssociation.setProjectId(oldAssociation.getProjectId());
				newAssociation.setRevisionId(oldAssociation.getRevisionId());
				newAssociation.setSolutionId(oldAssociation.getSolutionId());
				newAssociation.setUpdateTimestamp(oldAssociation.getUpdateTimestamp());
				newAssociation.setUserId(oldAssociation.getUserId());
				newAssociation.setVersion(oldAssociation.getVersion());
				couchDbService.editAssociation(newAssociation);
				predictor = getPredictorVO(authenticatedUserId,mlpUser,newAssociation);
			}
		} catch (Exception e) {
	...
	public ServiceState deleteAssociation(String authenticatedUserId, String associationId) {
		...
		PredictorProjectAssociation association= couchDbService.getAssociatedPredictor(associationId);
