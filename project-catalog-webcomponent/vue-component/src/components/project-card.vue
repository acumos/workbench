<template>
  <div
    ...
    style="width: 21rem;"
  >
    <span class="text-purple-500 font-semibold text-xl"
      >{{ project.name | truncate(22) }}
      <FAIcon
        class="mx-2 text-gray-800"
        icon="users"
        v-if="project.collaborators"
    /></span>

    <div class="flex">
      <span class="w-2/5 font-bold mx-1 text-right">ID:</span
      ><span class="flex-grow">{{ project.id }}</span>
    </div>
    <div class="flex">
      <span class="w-2/5 text-right font-bold mx-2">Version:</span>
      <span class="flex-grow">{{ project.version }}</span>
    </div>
    <div class="flex">
      <span class="w-2/5 text-right font-bold mx-2">Status:</span
      ><span :class="statusClass">{{ project.status | capitalize }}</span>
    </div>
    <div class="flex">
      <span class="w-2/5 text-right font-bold mx-2">Creation Date:</span
      ><span>{{ createdAt }}</span>
    </div>
    <div class="flex">
      <span class="w-2/5 text-right font-bold mx-2">Modified Date:</span
      ><span>{{ modifiedAt }}</span>
    </div>
    <div class="flex bg-gray-200 py-3 mt-2 px-2 justify-between">
      <div>
        <button class="mx-1" v-tooltip="project.owner">
          <FAIcon class="text-2xl text-gray-600" icon="user-circle"></FAIcon>
        </button>
        <button
          class="mx-1"
          v-if="currentUser !== project.owner"
          v-tooltip="currentUser"
        >
          <FAIcon class="text-2xl text-gray-600" icon="user-circle"></FAIcon>
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    openProject(project) {
      this.$emit("onOpenProject", project);
    },
