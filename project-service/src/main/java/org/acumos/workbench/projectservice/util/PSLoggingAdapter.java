...
import org.acumos.workbench.projectservice.util.PSLogConstants.MDCs;
import org.acumos.workbench.projectservice.util.PSLogConstants.ResponseSeverity;
...
public class PSLoggingAdapter {
	 ...
	 *            non-null logger.
	 */
	public PSLoggingAdapter(final Logger logger) {
	 ...
	 * @return this.
	 */
	public PSLoggingAdapter entering(final RequestAdapter<?> request, String format, Object... arguments) {
	 ...
	 * @return this.
	 */
	public PSLoggingAdapter entering(final HttpServletRequest request) {
	 ...
	 * @param format
	 * @param arguments
	 * @return PSLoggingAdapter
	 */
	public PSLoggingAdapter entering(final HttpServletRequest request, String format, Object... arguments) {
	 ...
	 * @return this.
	 */
	public PSLoggingAdapter exiting() {
	 ...
	 * @return this.
	 */
	public PSLoggingAdapter setResponseDescriptor(final ResponseDescriptor d) {
	 ...
	 * @return this.
	 */
	public PSLoggingAdapter setEnteringMDCs(final RequestAdapter<?> request) {

		// Extract MDC values from standard HTTP headers.
		final String requestID = defaultToUUID(request.getHeader(PSLogConstants.Headers.REQUEST_ID));
		final String partnerName = defaultToEmpty(request.getHeader(PSLogConstants.Headers.PARTNER_NAME));
		final String targetEntity = defaultToEmpty(request.getHeader(PSLogConstants.MDCs.TARGET_ENTITY));
		final String targetServiceName = defaultToEmpty(request.getHeader(PSLogConstants.MDCs.TARGET_SERVICE_NAME));

		// Get the UserName
		final String userName = defaultToEmpty(request.getUser());
		MDC.put(PSLogConstants.MDCs.USER, userName);

		PSLoggingAdapter.setDefaultMDCs();

		// Set standard MDCs.
		MDC.put(PSLogConstants.MDCs.REQUEST_ID, requestID);
		if (!partnerName.isEmpty())
			MDC.put(PSLogConstants.MDCs.PARTNER_NAME, partnerName);
		MDC.put(PSLogConstants.MDCs.CLIENT_IP_ADDRESS, defaultToEmpty(request.getClientAddress()));
		MDC.put(PSLogConstants.MDCs.SERVER_FQDN, defaultToEmpty(request.getServerAddress()));

		MDC.put(PSLogConstants.MDCs.TARGET_ENTITY, targetEntity);
		MDC.put(PSLogConstants.MDCs.TARGET_SERVICE_NAME, targetServiceName);
		return this;
	}
	...
	public static class ResponseDescriptor {
		...
		protected PSLogConstants.ResponseStatus mStatus;
		 ...
		 * @return this.
		 */
		public ResponseDescriptor setResponseStatus(final PSLogConstants.ResponseStatus status) {
			this.mStatus = status;
			return this;
		}
		...
		public void setMDCs() {
			...
			if (!response.isEmpty())
				MDC.put(PSLogConstants.MDCs.RESPONSE_CODE, response);
			if (!description.isEmpty())
				MDC.put(PSLogConstants.MDCs.RESPONSE_DESCRIPTION, description);
			if (!severity.isEmpty())
				MDC.put(PSLogConstants.MDCs.RESPONSE_SEVERITY, severity);
			if (!status.isEmpty())
				MDC.put(PSLogConstants.MDCs.RESPONSE_STATUS_CODE, status);
		}
	}
	...
	public static class HttpServletRequestAdapter implements RequestAdapter<HttpServletRequestAdapter> {
		...
		public Object getUser() {
			return this.mRequest.getAttribute(PSLogConstants.MDCs.USER);
		}
	}
}