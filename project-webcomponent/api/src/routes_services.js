...
module.exports = function(app) {
	...
	var getSelectedProjectPipelineDetails = function(userName, srvcUrl, projectId, authToken){
		return new Promise(function(resolve, reject) {
			...
			request.get(options, function(error, response) {
				if (!error && response.statusCode == 200) {						
					resolve(prepRespJsonAndLogit(response, JSON.parse(response.body), "Pipelines for the project retrieved successfully."));
				} else if (!error) {
					resolve(prepRespJsonAndLogit(response, response.body, "Unable to retrieve Pipelines."));
				} else {
					resolve(prepRespJsonAndLogit(null, null, null, error));
				}
			});
	...
	var createPipeline = function(userName, srvcUrl, projectId, newPipelineReq, authToken){
		...
		request.post(options, function(error, response, body) {
				if (!error && response.statusCode == 201) {
					resolve(prepRespJsonAndLogit(response, response.body, "Pipeline created successfully."));
				} else if (!error) {
					resolve(prepRespJsonAndLogit(response, response.body, "Unable to create Pipeline."));
				} else {
					resolve(prepRespJsonAndLogit(null, null, null, error));
				}
		});
	...
	var associatePipelineToProject = function(userName, srvcUrl, projectId, pipelineId, pipelinePayload, authToken){
		return new Promise(function(resolve, reject) {
			...
			json: true
					...
					request.put(options, function(error, response) {
			if (!error && response.statusCode == 200) {
				resolve(prepRespJsonAndLogit(response, response.body, "Pipeline associated successfully."));
			} else if (!error) {
				resolve(prepRespJsonAndLogit(response, response.body, "Unable to associate Pipeline."));
			} else {
				resolve(prepRespJsonAndLogit(null, null, null, error));
			}
					});
	...
	var archivePipeline = function(userName, srvcUrl, projectId, pipelineId, authToken){
		return new Promise(function(resolve, reject) {
			...
			request.put(options, function(error, response, body) {
				if (!error && response.statusCode == 200) {
					resolve(prepRespJsonAndLogit(response, JSON.parse(response.body), "Pipeline archived successfully."));
				} else if (!error) {
					resolve(prepRespJsonAndLogit(response, response.body, "Unable to archive Pipeline."));
				} else {
					resolve(prepRespJsonAndLogit(null, null, null, error));
				}
			});
	...
	var restorePipeline = function(userName, srvcUrl, projectId, pipelineId, authToken){
		return new Promise(function(resolve, reject) {
			...
			request.put(options, function(error, response, body) {
				if (!error && response.statusCode == 200) {
					resolve(prepRespJsonAndLogit(response, JSON.parse(response.body), "Pipeline restored successfully."));
				} else if (!error) {
					resolve(prepRespJsonAndLogit(response, response.body, "Unable to restore Pipeline."));
				} else {
					resolve(prepRespJsonAndLogit(null, null, null, error));
				}
			});
	...
	var getPipelinesList = function(userName, url, authToken) {
		return new Promise(function(resolve, reject) {
			...
			request.get(options, function(error, response) {
				if (!error && response.statusCode == 200) {
					resolve(prepRespJsonAndLogit(response, JSON.parse(response.body), "Pipelines retrieved successfully."));
				} else if (!error) {
					resolve(prepRespJsonAndLogit(response, response.body, "Unable to retrieve Pipelines."));
				} else {
					resolve(prepRespJsonAndLogit(null, null, null, error));
