...
export class ProjectLitElement extends DataMixin(ValidationMixin(BaseElementMixin(LitElement))) {
  ...
  static get properties() {
    ...
    renderViewProject(project){
      this.$data.set('project.projectName', project.projectId.name, true);
		  this.$data.set('project.projectCreateDate', project.projectId.versionId.timeStamp, true);
		  this.$data.set('project.projectDesc', project.description, true);
	    this.$data.set('project.projectModifyDate', project.projectId.versionId.timeStamp, true);
      this.$data.set('project.projectVersion', project.projectId.versionId.label, true);
      this.$data.set('project.projectStatus', project.artifactStatus.status, true);
      this.$data.set('project.projectId', project.projectId.uuid, true);
      ...
      fetch(url, {
        ...
        .then((n) => {
          if(n.status === 'Success'){
            ...
            this.projectName = this.data.project.projectName;
            this.$validations.resetValidation('project');
                  ...
                  <div class="row" style="margin:5px 0; margin-top: 0px;">
                    ...
                    ${this.data.project.projectStatus === 'ACTIVE'
                      ? html`
                        ${this.isEdit 
                          ? html`
                            ${this.$validations.$valid('project') && this.$validations.$dirty
                              ? html`
                        ...
                        ${this.isEdit 
                          ...
                          <td>
                            <textarea class="form-control" id="description" rows="2" .value=${this.data.project.projectDesc}
                              @keyup=${(e) => this.$data.set('project.projectDesc', e)}>
                            </textarea>
                          </td>
                          `
                          : html`
