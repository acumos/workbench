      ...
      <div v-if="!isEmpty">
        <div class="flex justify-end my-3">
          <div class="flex inline-flex items-center">
            ...
            <button class="btn btn-secondary text-black mr-2" @click="associateModel()">
              <FAIcon icon="link" />
            </button>
          </div>
        </div>
          ...
          <template slot="table-row" slot-scope="props">
            ...
            <div v-else-if="props.column.field === 'actions'">
              <div class="flex justify-center">
                <button class="btn btn-xs btn-primary mx-1" @click="editModelAssociation(props.row)">
                  <FAIcon icon="pencil-alt" />
                </button>
                <button class="btn btn-xs btn-secondary text-black mx-1">
                  <FAIcon icon="eye" />
                </button>
                <button class="btn btn-xs btn-secondary text-black mx-1" @click="deleteModelAssociation(props.row)">
                  <FAIcon icon="unlink" />
                </button>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    //         if(this.modelCategories[i].code === model.modelType){
    //           model.modelType = this.modelCategories[i].name;
    //         }
    //       }
    //     } else{
    //       model.modelType = "Others";
    //     }
    //     if(model.modelCatalog === "None"){
    //       model.modelCatalog = "Private Catalog";
    //     }
    //     return model;
    //   })
    // }
  ...
  methods: {
    ...
    async deleteModelAssociation(model){
      ...
      const statusMessage = await this.deleteAssociation(model.$updateToJson());
