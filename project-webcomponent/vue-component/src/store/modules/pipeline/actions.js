...
import { map, differenceWith } from "lodash-es";
...
export default {
  async getAllPipelines({ rootState }) {
    const { data } = await axios.post(
      ...
      const allPipelines = data.data.map(pipeline => Pipeline.$fromJson(pipeline));
      const projectPipelines = Pipeline.all();
      return differenceWith(allPipelines, projectPipelines, (pipeline, projectPipeline) =>
        pipeline.id === projectPipeline.id && pipeline.version === projectPipeline.version
      )
  },
